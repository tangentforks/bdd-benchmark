...
template <>
int
run_apply<bex_bdd_adapter>(int argc, char** argv)
{
  ...
  return run<bex_bdd_adapter>("apply", 0, [&](bex_bdd_adapter& adapter) {
    std::cout << json::field("inputs") << json::array_open << json::endl;

    // =============================================================================================
    // Load DDs
    std::vector<bex_bdd_adapter::dd_t> inputs_dd;
    ...
    // =============================================================================================
    // Apply DDs together
    bex_bdd_adapter::dd_t result = inputs_dd.at(0);

    std::cout << json::field("apply") << json::brace_open << json::endl << json::flush;

    const time_point t_apply_before = now();
    for (size_t i = 1; i < inputs_dd.size(); ++i) {
      switch (oper) {
      case operand::AND: result = adapter.apply_and(result, inputs_dd.at(i)); break;
      case operand::OR: result = adapter.apply_or(result, inputs_dd.at(i)); break;
      }
    }
...
{
  return run_apply<bex_bdd_adapter>(argc, argv);
}
